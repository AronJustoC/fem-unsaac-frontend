...
const StructureEditor: React.FC<StructureEditorProps> = ({ onVisualize }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-bg-dark-panel rounded-[2.5rem] border border-unsaac-red/20 dark:border-brand-navy/30 overflow-hidden shadow-2xl transition-all duration-500">
      ...
      <nav className="flex px-6 py-3 gap-1.5 bg-gray-50/50 dark:bg-black/40 border-b border-unsaac-red/20 dark:border-brand-navy/30 overflow-x-auto no-scrollbar">
        {[
          {
            id: "nodes",
            label: "Nodos",
          ...
          },
          {
            ...
            <button
              ...
              className={`flex items-center gap-3 px-5 py-2.5 rounded-xl text-[10px] font-display font-black uppercase tracking-widest transition-all cursor-pointer whitespace-nowrap ${
                activeTab === tab.id

                ? `bg-white dark:bg-bg-dark text-gray-900 dark:text-white shadow-md border border-unsaac-gold/50 dark:border-unsaac-gold/50 ring-1 ring-unsaac-gold/20`
                ...
                {data.nodes.map((node) => (
                  <div key={node.id} className="relative group">
                    <button
                      ...
                      className="w-full flex flex-col items-center justify-center bg-gray-50 dark:bg-black/20 p-6 rounded-[2rem] border border-border-light dark:border-border-dark hover:border-unsaac-gold/50 hover:shadow-2xl hover:shadow-unsaac-gold/5 transition-all duration-300 cursor-pointer"
                    >
                  ...
                  {data.materials.map((m) => (
                    <div key={m.id} className="relative group">
                      ...
                      >
                        ...
                        <span className="text-sm font-black text-gray-900 dark:text-white mb-2 truncate max-w-full px-2">
                          {m.name.toUpperCase()}
                        </span>
                ...
                {data.loads.map((load) => (
                  <div key={load.id} className="relative group">
                    <button
                      ...
                      className="w-full flex flex-col items-start bg-gray-50 dark:bg-black/20 p-6 rounded-[2rem] border border-border-light dark:border-border-dark hover:border-unsaac-gold/50 hover:shadow-2xl hover:shadow-unsaac-gold/5 transition-all duration-300 cursor-pointer"
                    >
                      ...
                      <span className="text-lg font-black text-gray-900 dark:text-white mb-4 truncate w-full text-left">
                        {load.name}
                      </span>
                      ...
                      <div className="grid grid-cols-3 gap-2 w-full">
                        {[
                          ...
                          .map((c) => (
                            <div
                              key={c.label}
                              className="flex items-center gap-1.5 px-2 py-1 bg-white dark:bg-bg-dark rounded-lg border border-border-light dark:border-border-dark shadow-sm"
                            >
